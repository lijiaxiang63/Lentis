...
| File | Role |
|---|---|
| `App.swift` | `@main LentisApp`. Menus, `--benchmark <path>` auto-open, plain `WindowGroup` (title via `.navigationTitle`), owns `UpdaterController` (`@StateObject`); app-menu **Check for Updates…**. File menu **Close (⌘W)** calls `model.requestClose()` (closes the file, keeps the window open — an explicit key equivalent in the `replacing: .newItem` group claims ⌘W over the system close-window default). |
...
| `ViewerModel.swift` (~1700 lines) | Central `@ObservableObject`. Panels, volume cache, MPR/3D, W/L, sync-scroll. Crosshair (`setCrosshair` — `.volume3D` excluded) backed by a decoupled `CrosshairState`. 3D (`loadVolumeRendering` async/coalesced on `panel.loadingQueue`). W/L drag re-drives `loadMPRSlice`/`loadVolumeRendering` off-main. **File close/replace:** `closeCurrentFile()` returns to the empty launch state (clears `allSeries`/`panels`/`niftiDataset`/`_volumeCache`/`seriesStates`/layers/segmentation, cancels in-flight queues); `requestClose()`/`requestLoad(url:)` gate on `hasUnsavedWork` + the `confirmReplaceOnDiscard` preference, staging a `PendingConfirmation` the ContentView alert performs/cancels. |
...
| `AppSettings.swift` | App-wide preferences (UserDefaults-backed `ObservableObject` singleton): FreeSurfer home / `mri_synthseg` path, SynthSeg flags + threads, output-location mode (next-to-source / custom / BIDS derivatives) + auto-load + write-brain-mask, overlay opacity, export filename suffixes, `confirmReplaceOnDiscard` (close/replace confirmation gate). `resolveOutputDirectory` (pure, tested) picks a writable dir with fallbacks. |
| `SettingsView.swift` | The Settings window (⌘,). `TabView`: **General** (overlay opacity, output location + folder picker with a live "Files go to" preview, auto-load / write-brain-mask, **Export File Names** builder rows + live both-files Preview, **File Handling** → `confirmReplaceOnDiscard` toggle) and **FreeSurfer** (live `mri_synthseg` status, home/binary pickers, `--robust`/`--parc`/threads). |
...
| `ContentView.swift` | Root `NavigationSplitView` (sidebar + detail) + native trailing `.inspector` (`LayerInspectorView`). Detail = `ZStack` { panel grid · floating glass `ToolPalette` · `NiftiLoadingOverlay` · `ViewerStatusBar` } + `.navigationTitle`/`.navigationSubtitle` + `.toolbar { ViewerToolbar }`. Owns the closed-state "Show Layers Inspector" toolbar button. Drag-to-open routes through `model.requestLoad(url:)` (confirms before discarding unsaved work). Two alerts: layer-import error + the close/replace `PendingConfirmation`. Global `.tint(.lentisAccent)`; `.preferredColorScheme(.dark)`. |
...
- [x] **UI polish + brush undo + ROI resize cursor (PR #8, 2026-06-27).**
  ...
  - **Pan tool repair:** dead Pan tool fixed via absolute-coordinate deltas; greyed out
    on 3D.
- [x] **Panel drop replacement confirmation (2026-06-27).** File/folder drops onto
  rendered image panels now route through `requestLoad`, matching the ContentView drop
  path and preserving the unsaved-work replace confirmation gate. Version bumped to
  **2.2.1**.
...
- [x] **Close current file + drag-to-replace confirmation (2026-06-27).**
  - **File → Close (⌘W):** `ViewerModel.closeCurrentFile()` returns the viewer to the
    empty "No file open" launch state — clears `allSeries`/`panels`/`niftiDataset`/
    `loadedFileName`/`loadedFileURL`/`dataset`/`_volumeCache`/`seriesStates`/layers/
    segmentation, cancels in-flight `cachingQueue` + per-panel `loadingQueue`s, and
    resets the layout to `.single`. The window stays open (the traffic light / ⌘Q
    still close/quit); ⌘W is claimed by an explicit key equivalent in the
    `replacing: .newItem` File-menu group so it closes the file, not the window.
  - **Drag-to-replace confirmation:** `ContentView.handleDrop` now routes through
    `model.requestLoad(url:)` instead of `load` directly. `requestLoad` / `requestClose`
    gate on `hasUnsavedWork` (committed `calcRegions`, an in-flight `draftRegion`,
    external `layerStore.layers`, or a `brainMaskLayer`) AND the new
    `AppSettings.confirmReplaceOnDiscard` preference (on by default). When both hold,
    a `PendingConfirmation` is staged; the `ContentView` alert performs or cancels it.
    No prior file → no prompt (first open is silent); preference off → silent replace.
  - **Settings:** General tab gains a **File Handling** section with the
    `confirmReplaceOnDiscard` toggle + explanatory footer.
  - **Tests:** `CloseFileTests` (16 XCTest) cover `closeCurrentFile` state reset +
    idempotency + cache release, `hasUnsavedWork` across all four triggers, and the
    `requestClose`/`requestLoad` confirmation gate (prompts / silent / perform /
